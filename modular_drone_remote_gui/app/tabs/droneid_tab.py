...
from app.maps import DRONEID_HTML
from app.readers.serial_reader import SerialReader
from app.utils.geo import distance_m, bearing_deg_true_north
from app.utils.webview import make_webview


class DroneIdTab(QWidget):
    def __init__(self, detector_state, port: str = "", baud: int = 115200):
        super().__init__()
        ...
        self.drone_tracks: Dict[str, Dict[str, Any]] = {}
        self.max_detector_range_m: float = 10000.0

        self.web = make_webview(self)
        self.web.setHtml(DRONEID_HTML)
        ...
        for lbl in [
            self.lbl_status, self.lbl_type, self.lbl_freq, self.lbl_time,
            self.lbl_track, self.lbl_controller, self.lbl_drone,
            self.lbl_alt, self.lbl_detector, self.lbl_drone_vec,
            self.lbl_controller_vec
        ]:
            lbl.setTextInteractionFlags(Qt.TextSelectableByMouse)

        left_form = QFormLayout()
        left_form.addRow("Status:", self.lbl_status)
        left_form.addRow("Device type:", self.lbl_type)
        left_form.addRow("Frequency (MHz):", self.lbl_freq)
        left_form.addRow("Drone GPS time:", self.lbl_time)
        left_form.addRow("Track key:", self.lbl_track)
    ...
    def start_reader(self):
        ...
        if not port:
            QMessageBox.warning(self, "Missing COM Port", "Select a COM port for the ANTSDR serial feed.")
            return
        self.reader = SerialReader(port, self.default_baud)
        self.reader.got_payload.connect(self.on_payload)
        self.reader.got_line.connect(self.on_line)
    ...
    def on_error(self, msg: str):
        ...
        QMessageBox.critical(self, "DroneID Error", msg)
    ...
    def _passes_basic_filter(self, p: Dict[str, Any]) -> bool:
        valid_points = []
        for lat_key, lon_key in (("app_lat", "app_lon"), ("drone_lat", "drone_lon")):
        ...
        if det_lat is not None and det_lon is not None:
            in_range = any(
                self._within_detector_range(det_lat, det_lon, lat, lon, self.max_detector_range_m)
    ...
    @staticmethod
    def _track_key(payload: Dict[str, Any]) -> str:
        for key in ("drone_id", "serial_number", "serial", "device_sn", "drone_sn", "mac"):
    ...
    def _match_existing_track(self, payload: Dict[str, Any]) -> str:
        ...
        if device_type:
            return device_type
        explicit = self._track_key(payload)
        if explicit:
            return explicit
    ...
    def _refresh_detected_dropdown(self):
        ...
        keys = sorted(self.drone_tracks.keys())
        self.cmb_detected.blockSignals(True)
        self.cmb_detected.clear()
        self.cmb_detected.addItem("None")
    ...
    def _update_selected_drone_table(self, track_key: Optional[str]):
        values = ["", "", ""]
        if track_key and track_key != "None" and track_key in self.drone_tracks:
            track = self.drone_tracks[track_key]
            values = [
    ...
    def on_payload(self, p: Dict[str, Any]):
        ...
        self.lbl_type.setText(str(p.get("device_type", "-")))
        self.lbl_freq.setText(str(p.get("freq", "-")))
        self.lbl_time.setText(str(p.get("drone_gps_time", "-")))
        self.lbl_track.setText(track_key)
        self.lbl_controller.setText(f"{self._f(p.get('app_lat'))}, {self._f(p.get('app_lon'))}".strip(", "))
        self.lbl_drone.setText(f"{self._f(p.get('drone_lat'))}, {self._f(p.get('drone_lon'))}".strip(", "))
        self.lbl_alt.setText(str(p.get("altitude", "-")))
        ...
        controller_dist, controller_brg = self._distance_and_bearing(
            self._as_float(p.get("app_lat")), self._as_float(p.get("app_lon"))
        )
        ...
        set_cell(2, str(p.get("device_type", "")))
        set_cell(3, str(p.get("freq", "")))
        set_cell(4, self._f(p.get("app_lat")))
        set_cell(5, self._f(p.get("app_lon")))
        set_cell(6, self._f(p.get("drone_lat")))
        set_cell(7, self._f(p.get("drone_lon")))
        ...
        if self.store is not None:
            self.store.update_droneid(dict(p))